...
## Setup
Prior toward running this code, you need to first install PharmacoGx (Task01) and  download teh relevant datasets (Task02)
```{r}
  library(PharmacoGx)
  library(tidyverse)
```
...
```{r}
  ...
  ic50s = data.frame(
    CCLE = as.vector(summarizeSensitivityProfiles(common$CCLE,sensitivity.measure = measure)),
    GDSC = as.vector(summarizeSensitivityProfiles(common$GDSC,sensitivity.measure = measure)))
...
```{r}
  ...
  ggplot(cor.cor,aes(x=CCLE,y=GDSC,color=Sign)) +
    geom_point() + 
    geom_smooth(aes(color=NULL),show.legend = FALSE,method='loess',show.legend=FALSE) +
